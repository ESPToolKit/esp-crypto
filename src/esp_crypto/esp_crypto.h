...
class AesCtrStream {
  public:
	...
	~AesCtrStream();
	CryptoStatusDetail
	begin(const std::vector<uint8_t> &key, CryptoSpan<const uint8_t> nonceCounter);
...
class ESPCrypto {
  public:
	...
	sha(const uint8_t *data, size_t length, const ShaOptions &options = ShaOptions{});
	static std::vector<uint8_t>
	sha(const std::vector<uint8_t> &data, const ShaOptions &options = ShaOptions{});
	static CryptoResult<std::vector<uint8_t>>
	shaResult(CryptoSpan<const uint8_t> data, const ShaOptions &options = ShaOptions{});
	static CryptoResult<void>
	sha(CryptoSpan<const uint8_t> data,
	    CryptoSpan<uint8_t> out,
	    const ShaOptions &options = ShaOptions{});

	static String
	shaHex(const uint8_t *data, size_t length, const ShaOptions &options = ShaOptions{});
	...
	static CryptoResult<GcmMessage> aesGcmEncryptAuto(
	    const std::vector<uint8_t> &key,
	    const std::vector<uint8_t> &plaintext,
	    const std::vector<uint8_t> &aad = {},
	    size_t ivLength = 12,
	...
	static CryptoResult<std::vector<uint8_t>> aesGcmDecrypt(
	    const std::vector<uint8_t> &key,
	    const std::vector<uint8_t> &iv,
	    const std::vector<uint8_t> &ciphertext,
	    const std::vector<uint8_t> &tag,
	...
	static CryptoResult<void> aesGcmEncrypt(
	    const std::vector<uint8_t> &key,
	    CryptoSpan<const uint8_t> iv,
	...
	static CryptoResult<void> aesGcmDecrypt(
	    const std::vector<uint8_t> &key,
	    CryptoSpan<const uint8_t> iv,
	...
	static CryptoResult<std::vector<uint8_t>> aesCtrCrypt(
	    const std::vector<uint8_t> &key,
	    const std::vector<uint8_t> &nonceCounter,
	    const std::vector<uint8_t> &input
	);
	...
	static bool rsaSign(
		...
		return rsaSign(
		    std::string(privateKeyPem.c_str(), privateKeyPem.length()),
		    data,
		    length,
		    variant,
		    signature
	...
	static bool rsaVerify(
		...
		return rsaVerify(
		    std::string(publicKeyPem.c_str(), publicKeyPem.length()),
		    data,
		    length,
		    signature,
		    variant
	...
	static CryptoResult<std::vector<uint8_t>>
	rsaSign(const std::string &privateKeyPem, CryptoSpan<const uint8_t> data, ShaVariant variant);
	static CryptoResult<void> rsaVerify(
	    const std::string &publicKeyPem,
	...
	static CryptoResult<std::vector<uint8_t>>
	eccSign(const std::string &privateKeyPem, CryptoSpan<const uint8_t> data, ShaVariant variant);
	static CryptoResult<void> eccVerify(
	    const std::string &publicKeyPem,
	    CryptoSpan<const uint8_t> data,
	    CryptoSpan<const uint8_t> signature,
	    ShaVariant variant
	...
	static CryptoResult<String> createJwtResult(
	    const JsonDocument &claims,
	    const std::string &key,
	    const JwtSignOptions &options = JwtSignOptions{}
	);
	static CryptoResult<void> verifyJwtResult(
	    const String &token,
	    const std::string &key,
	    JsonDocument &outClaims,
	    const JwtVerifyOptions &options = JwtVerifyOptions{}
	);
	static CryptoResult<void> verifyJwtWithJwks(
	    const String &token,
	...
	static String
	hashString(const String &input, const PasswordHashOptions &options = PasswordHashOptions{});
	static bool verifyString(const String &input, const String &encoded);

	static CryptoResult<String> hashStringResult(
	    const String &input, const PasswordHashOptions &options = PasswordHashOptions{}
	);
	...
	static CryptoResult<std::vector<uint8_t>> hkdf(
	    ShaVariant variant,
	...
	static CryptoResult<std::vector<uint8_t>> pbkdf2(
	...
	static CryptoResult<std::vector<uint8_t>> deriveDeviceKey(
	...
	static CryptoResult<void>
	storeKey(KeyStore &store, const KeyHandle &handle, CryptoSpan<const uint8_t> keyMaterial);
	static CryptoResult<CryptoKey> loadKey(
	    KeyStore &store, const KeyHandle &handle, KeyFormat format, KeyKind kind = KeyKind::Auto
	);
	...
	static CryptoResult<std::vector<uint8_t>> chacha20Poly1305Encrypt(
	    CryptoSpan<const uint8_t> key,
	    CryptoSpan<const uint8_t> nonce,
	    CryptoSpan<const uint8_t> aad,
	    CryptoSpan<const uint8_t> plaintext
	);
	static CryptoResult<std::vector<uint8_t>> chacha20Poly1305Decrypt(
	    CryptoSpan<const uint8_t> key,
	    CryptoSpan<const uint8_t> nonce,
	    CryptoSpan<const uint8_t> aad,
	    CryptoSpan<const uint8_t> ciphertextAndTag
	);
	static CryptoResult<std::vector<uint8_t>> xchacha20Poly1305Encrypt(
	    CryptoSpan<const uint8_t> key,
	    CryptoSpan<const uint8_t> nonce,
	    CryptoSpan<const uint8_t> aad,
	    CryptoSpan<const uint8_t> plaintext
	);
	static CryptoResult<std::vector<uint8_t>> xchacha20Poly1305Decrypt(
	    CryptoSpan<const uint8_t> key,
	    CryptoSpan<const uint8_t> nonce,
	    CryptoSpan<const uint8_t> aad,
	    CryptoSpan<const uint8_t> ciphertextAndTag
	...
	static CryptoResult<std::vector<uint8_t>>
	ed25519Sign(CryptoSpan<const uint8_t> privateKey, CryptoSpan<const uint8_t> message);
	static CryptoResult<void> ed25519Verify(
