...
void test_hkdf_rfc5869_case1() {
	...
	const uint8_t expected[] = {0x3c, 0xb2, 0x5f, 0x25, 0xfa, 0xac, 0xd5, 0x7a, 0x90, 0x43, 0x4f,
...
void test_pbkdf2_vector() {
	...
	const uint8_t expected[] = {0x23, 0x1a, 0xfb, 0x7d, 0xcd, 0x2e, 0x86, 0x0c, 0xfd, 0x58, 0xab,
	                            0x13, 0x37, 0x2b, 0xd1, 0x2c, 0x92, 0x30, 0x76, 0xc3, 0x59, 0x8a,
	                            0x12, 0x19, 0x60, 0x32, 0x0f, 0x6f, 0xec, 0x8a, 0x56, 0x98};
	TEST_ASSERT_TRUE(
	    ESPCrypto::constantTimeEq(
	        derived.value,
	        std::vector<uint8_t>(expected, expected + sizeof(expected))
...
void test_jwt_roundtrip_hs256() {
	...
	TEST_ASSERT_TRUE_MESSAGE(
	    ESPCrypto::verifyJwt(token, "secret", decoded, error, verifyOptions),
	    error.c_str()
	);
...
void test_sha_ctx_streaming() {
	...
	const char *chunk2 = "c";
	TEST_ASSERT_TRUE(
	    ctx.update(CryptoSpan<const uint8_t>(reinterpret_cast<const uint8_t *>(chunk1), 2)).ok()
	);
	TEST_ASSERT_TRUE(
	    ctx.update(CryptoSpan<const uint8_t>(reinterpret_cast<const uint8_t *>(chunk2), 1)).ok()
	);
	...
	TEST_ASSERT_TRUE(
	    ESPCrypto::constantTimeEq(out, std::vector<uint8_t>(expected, expected + sizeof(expected)))
	);
}
...
void test_sha_ctx_rebegin_reuses_context() {
	...
	TEST_ASSERT_TRUE(ctx.begin(ShaVariant::SHA256).ok());
	TEST_ASSERT_TRUE(
	    ctx.update(CryptoSpan<const uint8_t>(reinterpret_cast<const uint8_t *>(input), 3)).ok()
	);
	...
	const uint8_t expectedSha256[] = {0xba, 0x78, 0x16, 0xbf, 0x8f, 0x01, 0xcf, 0xea,
	                                  0x41, 0x41, 0x40, 0xde, 0x5d, 0xae, 0x22, 0x23,
	                                  0xb0, 0x03, 0x61, 0xa3, 0x96, 0x17, 0x7a, 0x9c,
	                                  0xb4, 0x10, 0xff, 0x61, 0xf2, 0x00, 0x15, 0xad};
	const uint8_t expectedSha512[] = {0xdd, 0xaf, 0x35, 0xa1, 0x93, 0x61, 0x7a, 0xba, 0xcc, 0x41,
	...
	TEST_ASSERT_TRUE(
	    ESPCrypto::constantTimeEq(
	        sha256,
	        std::vector<uint8_t>(expectedSha256, expectedSha256 + sizeof(expectedSha256))
	    )
	);
	TEST_ASSERT_TRUE(
	    ESPCrypto::constantTimeEq(
	        sha512,
	        std::vector<uint8_t>(expectedSha512, expectedSha512 + sizeof(expectedSha512))
	    )
	);
}
...
void test_aes_ctr_stream_roundtrip() {
	...
	TEST_ASSERT_TRUE(
	    enc.update(CryptoSpan<const uint8_t>(plaintext), CryptoSpan<uint8_t>(ciphertext)).ok()
	);

	AesCtrStream dec;
	TEST_ASSERT_TRUE(dec.begin(key, CryptoSpan<const uint8_t>(nonce)).ok());
	TEST_ASSERT_TRUE(
	    dec.update(CryptoSpan<const uint8_t>(ciphertext), CryptoSpan<uint8_t>(decrypted)).ok()
	);
...
void test_aes_gcm_ctx_roundtrip() {
	...
	AesGcmCtx enc;
	TEST_ASSERT_TRUE(
	    enc.beginEncrypt(key, CryptoSpan<const uint8_t>(iv), CryptoSpan<const uint8_t>(aad)).ok()
	);
	TEST_ASSERT_TRUE(
	    enc.update(CryptoSpan<const uint8_t>(plaintext), CryptoSpan<uint8_t>(ciphertext)).ok()
	);
	...
	std::vector<uint8_t> decrypted(plaintext.size(), 0);
	AesGcmCtx dec;
	TEST_ASSERT_TRUE(dec.beginDecrypt(
	                        key,
...
void test_chacha20poly1305_roundtrip() {
	...
	auto enc = ESPCrypto::chacha20Poly1305Encrypt(
	    CryptoSpan<const uint8_t>(key),
	...
	TEST_ASSERT_TRUE_MESSAGE(enc.ok(), enc.status.message.c_str());
	auto dec = ESPCrypto::chacha20Poly1305Decrypt(
	    CryptoSpan<const uint8_t>(key),
	    CryptoSpan<const uint8_t>(nonce),
	    CryptoSpan<const uint8_t>(aad),
...
void test_aes_gcm_span_roundtrip() {
	...
	auto enc = ESPCrypto::aesGcmEncrypt(
	    key,
	    CryptoSpan<const uint8_t>(iv),
	    CryptoSpan<const uint8_t>(plaintext),
	...
	std::vector<uint8_t> decrypted(plaintext.size(), 0);
	auto dec = ESPCrypto::aesGcmDecrypt(
	    key,
	    CryptoSpan<const uint8_t>(iv),
...
void test_jwt_and_envelope_fuzz() {
	const char *badTokens[] =
	    {"", "abc", "a.b", "a.b.c", "e30=.e30=.@@@@", "eyJhbGciOiJIUzI1NiJ9.e30.bad"};
	JsonDocument out;
	String err;
