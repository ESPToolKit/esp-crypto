...
void setup() {
	...
	String digest =
	    ESPCrypto::shaHex(reinterpret_cast<const uint8_t *>(message.c_str()), message.length());
	...
	auto decrypted = ESPCrypto::aesGcmDecrypt(
	    key,
	    encrypted.value.iv,
	...
	Serial.printf(
	    "GCM plaintext recovered: %s\n",
	    String(reinterpret_cast<const char *>(decrypted.value.data()), decrypted.value.size())
	        .c_str()
	);
